...
class Client implements ClientInterface {
     ...
     */
    private function filterEmptyArrays(array $array): array {
     ...
     */
    protected function getResourceMethod(string $methodName): array {
     ...
     *   The request uri.
     */
    protected function createRequestUri(string $uri, array $params = []): string {
     ...
     * {@inheritdoc}
     */
    public function getRoute(string $name): array|bool {
        return $this->apiCall(__METHOD__, $name);
    }
     ...
     * {@inheritdoc}
     */
    public function getBuilds(string $name, string $label): array|bool {
     ...
     * {@inheritdoc}
     */
    public function getBuildConfig(string $name): array|bool {
        return $this->apiCall(__METHOD__, $name);
    }
     ...
     * {@inheritdoc}
     */
    public function updateBuildConfig(string $name, string $secret, string $image_stream, array $data): array {
     ...
     * {@inheritdoc}
     */
    public function getImageStream(string $name): array|bool {
        return $this->apiCall(__METHOD__, $name);
    }
     ...
     * {@inheritdoc}
     */
    public function getImageStreamTag(string $name): array|bool {
        return $this->apiCall(__METHOD__, $name);
    }
     ...
     * {@inheritdoc}
     */
    public function getPersistentVolumeClaim(string $name): array|bool {
        return $this->apiCall(__METHOD__, $name);
    }
     ...
     * {@inheritdoc}
     */
    public function getDeploymentConfig(string $label): array|bool {
        return $this->apiCall(__METHOD__, $label);
    }
     ...
     * {@inheritdoc}
     */
    public function getDeploymentConfigs(string $label = NULL): array|bool {
    ...
    public function generateDeploymentConfig(string $name, string $image_stream_tag, string $image_name,
                                             bool   $update_on_image_change = FALSE, array $volumes = [],
                                             array  $data = [], array $probes = []): array {
     ...
     */
    protected function generateSecurityContext($data): array {
        return [
            'securityContext' => [
                'runAsUser' => $data['uid'],
     ...
     */
    protected function generateProbeConfigs($probes): array {
        $probeConfigs = [];
     ...
     */
    protected function generateProbeConfig(array $probe): array {
        $probeConfig = [];
        switch ($probe['type']) {
            case 'exec':
                $probeConfig = [
     ...
     * {@inheritdoc}
     */
    public function instantiateDeploymentConfig(string $name): array|bool {
        $resourceMethod = $this->getResourceMethod(__METHOD__);
        $uri = $this->createRequestUri($resourceMethod['uri'], [
            'name' => $name,
        ]);
     ...
     * {@inheritdoc}
     */
    public function getCronJob(string $name, string $label = ''): array|bool {
     ...
     * {@inheritdoc}
     */
    public function getJob(string $name, string $label = ''): array|bool {
        return $this->apiCall(__METHOD__, $name, $label);
    }
     ...
     * {@inheritdoc}
     */
    public function getPods(): array|bool {
     ...
     * {@inheritdoc}
     */
    public function getPod(string $name, string $label): array|bool {
        return $this->apiCall(__METHOD__, $name, $label);
    }
     ...
     * {@inheritdoc}
     */
    public function getReplicationControllers(string $name, string $label): array|bool {
        return $this->apiCall(__METHOD__, $name, $label);
    }
     ...
     */
    public function updateReplicationControllers(string $name, string $label, int $replica_count): array|bool {
        $result = FALSE;
        ...
        if (isset($repControllers['items'])) {
            foreach ($repControllers['items'] as $controller) {
                $result = $this->updateReplicationControllers(
     ...
     * {@inheritdoc}
     */
    public function updateBackup(Backup $backup): ScheduledBackup|bool {
     ...
     * {@inheritdoc}
     */
    public function listRestore(Label $label_selector = NULL): RestoreList|bool {
        $label = NULL;
        if ($label_selector) {
            $label = (string)$label_selector;
        }
     ...
     * {@inheritdoc}
     */
    public function getSchedule(string $name): ScheduledBackup|bool {
        $result = $this->apiCall(__METHOD__, $name, NULL, FALSE);
        if (!$result) {
            return FALSE;
        }
     ...
     * {@inheritdoc}
     */
    public function createSchedule(ScheduledBackup $schedule): ScheduledBackup|bool {
     ...
     * {@inheritdoc}
     */
    public function updateSchedule(ScheduledBackup $schedule): ScheduledBackup|bool {
     ...
     * {@inheritdoc}
     */
    public function updateConfigmap(ConfigMap $configMap): ConfigMap|bool {
     ...
     * {@inheritdoc}
     */
    public function getConfigmap(string $name): ConfigMap|bool {
        $result = $this->apiCall(__METHOD__, $name, NULL, FALSE);
        if (!$result) {
            return FALSE;
        }
     ...
     * {@inheritdoc}
     */
    public function updateStatefulset(StatefulSet $statefulSet): StatefulSet|bool {
     ...
     * {@inheritdoc}
     */
    public function getStatefulset(string $name): StatefulSet|bool {
        $result = $this->apiCall(__METHOD__, $name, NULL, FALSE);
        if (!$result) {
            return FALSE;
        }
     ...
     * {@inheritdoc}
     */
    public function createHpa(Hpa $hpa): Hpa|bool {
     ...
     * {@inheritdoc}
     */
    public function getHpa(string $name): Hpa|bool {
        $result = $this->apiCall(__METHOD__, $name, NULL, FALSE);
        if (!$result) {
            return FALSE;
        }
     ...
     * {@inheritdoc}
     */
    public function updateHpa(Hpa $hpa): Hpa|bool {
     ...
     * {@inheritdoc}
     */
    public function getSync(string $name): Sync|bool {
        $result = $this->apiCall(__METHOD__, $name, NULL, FALSE);
        if (!$result) {
            return FALSE;
        }
     ...
     * {@inheritdoc}
     */
    public function listSync(Label $label_selector = NULL): SyncList|bool {
        $label = NULL;
        if ($label_selector) {
            $label = (string)$label_selector;
        }
     ...
     *
     */
    private function createSerializableObject(string $method, object $object, array $params = []): mixed {
     ...
     */
    private function applyAnnotations(array &$config, array $annotations): void {
     ...
     */
    private function setVolumes(array $volumes): array {
     ...
     */
    private function jobTemplate(string $name, string $image_name, array $args, array $volume_config,
                                 array  $data): array {
        $job_template = [
            'spec' => [
                'template' => [
     ...
     * @throws ClientException
     */
    private function apiCall(string $method, string $name = '', ?string $label = NULL,
                             bool   $decode_response = TRUE): bool|array {
        $resourceMethod = $this->getResourceMethod($method);
        $uri = $this->createRequestUri($resourceMethod['uri'], [
            'name' => (string)$name,
